(function ($) {
    ...
    window.accountList = new Array();
    
    watch(window.accountList, function () {
        var table = $('#accountList');
        table.children().remove();

        var rows = window.accountList.map(function (account, index) {
        ...
        if (window.accountList.length > 0) {
    ...
    $('#add-account').click(function (event) {
        ...
        window.accountList.push({accounts: account});
        clearFields();
    ...
    $('#remove-account').click(function (event) {
        ...
        $.each(accounts, function (index, row) {
            ...
            window.accountList.splice(indexOf, 1);
        });
    });
    ...
    $('#edit-account').click(function (event) {
        ...
        $.each(accounts, function (index, row) {
            ...
            window.accountList.splice($(row).attr('data-key'), 1);
        });
    });
